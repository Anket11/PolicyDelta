...
- `policydelta keys create` + org provisioning CLI
...
- `policydelta seed` — the six fixtures from ARCHITECTURE §8.3 (fake embeddings)
...
- Supersession suggest + `policydelta supersede` confirm (atomic close+edge) + **staleness flagger**
  over past runs
