# PolicyDelta — Production Architecture
...
- **Supersession: operator-confirmed, machine-suggested.** LLM `supersedes_refs` + fuzzy matcher
  propose; `policydelta supersede <new> --supersedes <old>` confirms — atomically closing intervals,
...
```text
policydelta ingest url|file <target> [--body SECP] [--jurisdiction PK] [--now]
policydelta worker run-once | run-forever        # cron target / local dev
policydelta status [--review] [--failed]
policydelta confirm <doc_id>
policydelta supersede <new_id> --supersedes <old_id> [--relation amends]
policydelta reindex <doc_id>
policydelta backfill-embeddings [--limit N]
policydelta retry <job_id>
policydelta seed                                  # dev fixtures (see §8.3)
policydelta keys create --org <id> --scopes audit  # tenant provisioning
```
...
### 8.3 Seed fixtures (`policydelta seed`)
