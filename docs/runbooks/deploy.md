...
1. Create a Neon project (Postgres 16), database `policydelta`. The default
...
uv run policydelta jurisdictions add PK "Pakistan"
uv run policydelta org create "FirstCustomer" --jurisdiction PK
uv run policydelta keys create --org-id 1 --scopes audit   # store the printed key NOW
...
| Audits stuck `queued` | Worker logs (`job_failed`, reaper `jobs_reaped`); `policydelta status --failed`; `policydelta retry <id>` |
| Document missing from search | `policydelta status --review` — quarantined? `policydelta confirm <doc_id>` after review |
