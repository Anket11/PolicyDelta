...
```powershell
uv run policydelta org create "Acme Fintech" --jurisdiction PK
uv run policydelta keys create --org-id <id> --name "prod-dashboard" --scopes audit
# admin-scope keys are for n8n / operators only:
uv run policydelta keys create --org-id <id> --name "n8n-monitor" --scopes admin
```
...
```powershell
uv run policydelta ingest url https://www.secp.gov.pk/.../circular-12-2026.pdf `
    --title "SECP Circular 12 of 2026" --body SECP --type Circular `
    --jurisdiction PK --published 2026-06-01
uv run policydelta worker run-once          # or let the in-process worker drain it
uv run policydelta status --review          # quarantined documents + reasons
uv run policydelta confirm <doc_id>         # operator judgment → retrievable
```
...
```powershell
uv run policydelta supersede <new_doc_id> --supersedes <old_doc_id> --relation amends
```
...
Any intentional API change: `uv run policydelta export-openapi`, review the
