# PolicyDelta — agent guide

Temporal Compliance RAG Engine. FastAPI backend in `apps/api` (src layout,
package `policydelta`); Next.js 16 dashboard in `apps/web` ("Gazette Ledger"
...
uv run policydelta seed      # idempotent 6-doc temporal fixture corpus + 2 orgs
...
6. **Contract freeze.** API changes require `uv run policydelta export-openapi`
