...
class Settings(BaseSettings):
    ...
    log_level: str = "INFO"
    app_name: str = "PolicyDelta Compliance API"
    ...
    database_url: str = (
        "postgresql+asyncpg://cg_app:cg_app_dev_password@localhost:5433/policydelta"
    )
    # Worker/CLI path: corpus writes + per-job tenant context.
    database_url_worker: str = (
        "postgresql+asyncpg://cg_worker:cg_worker_dev_password@localhost:5433/policydelta"
    )
    # Migrations only.
    database_url_owner: str = (
        "postgresql+asyncpg://cg_owner:cg_owner_dev_password@localhost:5433/policydelta"
    )
