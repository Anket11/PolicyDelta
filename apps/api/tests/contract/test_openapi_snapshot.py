...
intentional change: ``uv run policydelta export-openapi`` and commit the diff —
...
from policydelta.main import create_app
...
class TestContractFreeze:
    def test_live_schema_matches_committed_snapshot(self) -> None:
        ...
        assert snapshot_path.exists(), (
            ...
            "run `uv run policydelta export-openapi` and commit it."
        )
        committed = json.loads(snapshot_path.read_text(encoding="utf-8"))
        assert live == committed, (
            "OpenAPI contract drifted from packages/contracts/openapi.json. "
            "If intentional: `uv run policydelta export-openapi`, review the diff, commit."
        )
