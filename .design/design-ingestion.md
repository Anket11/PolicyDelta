...
# PolicyDelta — Ingestion Pipeline & Document Processing
...
**Scanned-vs-digital heuristic (MVP OCR stance = NONE):** open with PyMuPDF, compute extractable text characters per page. If `mean(chars_per_page) < 100` (mostly images), mark `is_scanned=True`, set job status `failed` with error `scanned_pdf_ocr_unsupported`, and surface in `policydelta status` for human handling. We do **not** attempt OCR in MVP.
...
3. **Verification:** assert `effective_date_evidence` is a substring of the source markdown (anti-hallucination). If it isn't, or `confidence != "high"`, or `effective_date is None`, the document is written with `extraction_status='review'` and **excluded from retrieval** until an operator confirms (`policydelta confirm <doc_id>`).

**Human review queue stance (MVP):** the queue is just `WHERE extraction_status='review'` — surfaced by `policydelta status --review` and (future) the Next.js admin view. No separate workflow engine. A reviewed doc flips to `confirmed`; `is_active` on its chunks is only set `true` on confirmation. This guarantees **no low-confidence regulatory text silently affects an audit** — the core correctness promise.
...
- The operator confirms via `policydelta supersede <new_id> --supersedes <old_id>`. On confirm, the backend:
...
- **Resumability (DECISION):** embeddings are written per-batch in their own transaction, setting `embedded_at`. A crash mid-document leaves earlier batches persisted. Resume logic selects only `WHERE document_id=? AND embedded_at IS NULL`. `policydelta backfill-embeddings` runs exactly this query corpus-wide — so partial failure is just "more rows to backfill", never a re-pay for done work.
...
Reasoning: the locked deployment is Neon + Railway/Render/Fly. FastAPI `BackgroundTasks` die with the request/dyno and have no retry/visibility — unacceptable for a multi-minute, money-spending pipeline. A full `arq`+Redis worker is the right *upgrade path* but adds a service to operate on day one. The middle ground: **`ingestion_jobs` is the durable queue**; a stateless `policydelta worker run-once` command leases queued jobs (`UPDATE ... SET locked_at=now() WHERE status='queued' ... FOR UPDATE SKIP LOCKED LIMIT 1`) and processes them. PaaS cron invokes `run-once` every minute; n8n webhooks insert jobs that the next tick drains.
...
- **Dead-letter stance:** `status='dead'` rows stay in the table (no separate DLQ infra); `policydelta status --dead` lists them; `policydelta retry <job_id>` re-queues.
...
```
policydelta ingest url   <URL>  [--body SECP] [--jurisdiction PK] [--type Circular] [--now]
policydelta ingest file  <PATH> [--body ...] [--jurisdiction ...]
policydelta worker run-once          # lease + process one queued job (PaaS cron target)
policydelta worker run-forever       # local dev loop (poll every 5s)
policydelta status        [--review] [--dead] [--job <id>]
policydelta confirm       <doc_id>                       # clear 'review', activate chunks
policydelta supersede     <new_id> --supersedes <old_id>
policydelta reindex       <doc_id>   # re-chunk from stored raw_markdown, new chunk rows
policydelta backfill-embeddings  [--limit N] [--max-tokens N]   # embed all embedded_at IS NULL
policydelta retry         <job_id>   # dead/failed → queued
```
...
**What n8n monitors:** RSS/index pages of SECP/SBP, diffing for new circular links on a nightly schedule; on a new link it POSTs the webhook. **What the backend owns:** download, hash/dedup, extract, chunk, metadata, embed, review-gating. n8n gets a fire-and-forget 202; status is polled via `policydelta status` / a future `/api/v1/ingest/jobs/{id}` endpoint.
