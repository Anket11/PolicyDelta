# Overengineering & MVP-Fit Critique — PolicyDelta
...
**Quoted (D4):** *"`policydelta worker run-once` … PaaS cron invokes `run-once` every minute."*
...
**Quoted:** *"Errors → RFC 9457 `application/problem+json` … `type_: str = "urn:policydelta:error:not-found"`."*
The full RFC 9457 envelope plus stable URN type slugs is a spec-compliance flourish. The *single consistent error shape* is justified (the UI/n8n need one shape — keep that). The `urn:policydelta:error:*` registry of type URIs is the cosplay — nobody dereferences them, and you'll maintain a slug catalog for no consumer.
