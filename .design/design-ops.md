...
# Ops, Quality & Delivery — PolicyDelta Compliance
...
services:
  db:
    ...
    environment:
      POSTGRES_USER: policydelta
      POSTGRES_PASSWORD: policydelta
      POSTGRES_DB: policydelta
    ports: ["5432:5432"]
    volumes: ["pgdata:/var/lib/postgresql/data"]
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U policydelta -d policydelta"]
      interval: 5s
      retries: 10
volumes: { pgdata: {} }
```
...
seed:      uv run python -m policydelta.cli seed-demo
run:       uv run uvicorn policydelta.main:app --reload
```
